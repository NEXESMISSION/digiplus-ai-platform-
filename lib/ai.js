...
// ─── the endpoint, and why it moved ──────────────────────────────────────────
//
// This was POST /v1/chat/completions with reasoning_effort "none" — the model told not to think.
// That was the right setting while its whole job was reading an id off a menu. The menu is gone and
// the job is now to work out what a client means, which is the one thing thinking is for, so the
// effort has to be raisable. It cannot be raised there. The endpoint says so itself — measured, not
// remembered:
//
//   POST /v1/chat/completions {reasoning_effort:"low", tools:[…]} → 400
...
//    'none'.»
//
// So the call moved to /v1/responses. What must not move with it is this function's contract:
// lib/reply.js reads .content and .tool_calls[].function off what comes back, pushes those same
// objects into `messages` as history, and tools/bench.js reads the usage by its chat/completions
// names. The Responses API speaks neither shape, so the request is translated on the way out and the
// answer is translated back on the way in, and the whole seam stays inside this file.

// Tools. Chat completions nests the function («{type:'function', function:{name,…}}»), Responses
// flattens it («{type:'function', name,…}»). lib/tools.js writes the nested one and is not this
// file's to change, so it is flattened here. A tool already flat is passed through: the day
// tools.js is rewritten this keeps working instead of sending {function: undefined}.
const toolsForResponses = (tools) => tools.map((tool) => (tool.type === 'function' && tool.function ? { type: 'function', ...tool.function } : tool));

// Messages → input items. Plain turns keep their role; the two chat-completions shapes that have no
// role in Responses are the tool round-trip, and they become items of their own:
//   {role:'assistant', tool_calls:[{id, function:{name, arguments}}]}  →  {type:'function_call', call_id, name, arguments}
//   {role:'tool', tool_call_id, content}                               →  {type:'function_call_output', call_id, output}
// The reasoning item that came with a function_call is deliberately NOT replayed. It can be — the
// response carries it encrypted when store is false — but that would mean holding model state in
// this module between calls, and a probe today confirmed the round-trip is accepted without it:
// function_call + function_call_output alone, no reasoning item, answered 200.
function toInput(messages) {
  ...
  for (const message of messages) {
    ...
    if (message.role === 'assistant' && message.tool_calls?.length) {
      // reply.js writes «content: reply.content || null» here, so the text is usually absent.
      if (content.trim()) input.push({ role: 'assistant', content });
      for (const call of message.tool_calls) {
        input.push({ type: 'function_call', call_id: call.id, name: call.function?.name, arguments: call.function?.arguments || '{}' });
      }
      continue;
    }
    // The rewrite round pushes «content: reply.content || ''». An empty assistant turn is nothing to
    // rewrite and Responses has no use for it; the developer message after it carries the order.
...
// The answer, back in the shape lib/reply.js already handles. Responses returns a list: reasoning
// items (skipped — they are the thinking, not the answer), message items whose content holds the
// output_text, and function_call items. Several messages are joined rather than picked between: the
// answer is one JSON object and dropping half of it would hand readAnswer a truncated one.
function toMessage(output) {
  ...
  const calls = items
    ...
    // call_id, not id: the id (fc_…) names the output item, call_id is what function_call_output
    // must carry back. reply.js puts this straight into tool_call_id, so the wrong one 400s a round
    // later, with the error naming a call the API never heard of.
...
// Usage, back under the chat/completions names. tools/bench.js:283 reads prompt_tokens,
// completion_tokens and prompt_tokens_details.cached_tokens off this — the cached count is what turns
// every cache estimate in the bench into a measured number — and bench.js is not this file's to
// change. reasoning_tokens is new and carried along: on this endpoint the thinking is billed as
// output, so it is the only way to see what an effort setting actually costs.
const toUsage = (usage) =>
  usage
    ? {
...
// On this endpoint the reasoning is billed out of the same budget as the answer, so a cap sized for
// the answer alone becomes a truncated answer the moment the effort goes up. 2500 was the answer's
// cap and it stays that; the thinking is given its own room on top of it.
const MAX_OUTPUT = REASONING && REASONING !== 'none' ? 2500 + 6000 : 2500;
...
async function complete(messages, { tools = [], toolChoice } = {}) {
  ...
  const body = {
    ...
    // Not `instructions`. The developer message stays inside the input, where reply.js put it,
    // because the json_object format is checked against the input and nothing else: with the same
    // text moved to `instructions` the call came back 400 «Response input messages must contain the
    // word 'json' in some form to use 'text.format' of type 'json_object'».
    text: { format: { type: 'json_object' } },
    max_output_tokens: MAX_OUTPUT,
    // Chat completions kept nothing. Responses stores the conversation by default, and what is in
    // these messages is a client's chat — their name, their phone number, their order.
    store: false,
  };
  if (tools.length) {
    body.tools = toolsForResponses(tools);
  ...
  // A failure this endpoint has and the old one did not: the run can stop on the token cap with the
  // thinking finished and the answer half-written, and it still arrives as 200. reply.js would then
  // throw «The AI returned nothing to send», which blames the model for a budget. Said out loud here.
  if (data.status && data.status !== 'completed') {
    console.warn(
      `lib/ai.js: the model stopped at "${data.status}" (${data.incomplete_details?.reason || 'no reason given'}) with ` +
        `${data.usage?.output_tokens ?? '?'} output tokens of ${MAX_OUTPUT}, ${data.usage?.output_tokens_details?.reasoning_tokens ?? '?'} of them thinking: ` +
        'the answer below is whatever was written before it stopped'
    );
  }
  const message = toMessage(data.output);
  trace?.({ messages, raw: message, usage: toUsage(data.usage) });
  return message;
}
