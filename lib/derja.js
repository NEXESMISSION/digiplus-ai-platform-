...
const FLAG = [
  ['mumkin', 'tnajjem / ynajjem'], ['momkin', 'tnajjem / ynajjem'],
  ...
  ['njah', 'ey'], ['na3am', 'ey'], ['tayeb', 'behi'], ['tamam', 'behi'], ['tamem', 'behi'],
  ['safi', 'behi / 5las'], ['wakha', 'ey / behi'], ['zwin', 'behi / mezyen'], ['zwina', 'behya / mezyena'],
  ['7elw', 'behi'], ['7elwa', 'behya'], ['ghir', 'ken / bark'], ['hna', 'houni'], ['hnaya', 'houni'], ['hne', 'houni'],
...
function fix(text) {
  ...
  for (const { wrong, right, pattern } of SWAPS) {
    out = out.replace(pattern, (match) => {
      changed.push(`${match} → ${right}`);
      return match[0] === match[0].toUpperCase() ? right[0].toUpperCase() + right.slice(1) : right;
    });
  }
