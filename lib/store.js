...
// A benchmark run keeps everything in memory instead (tools/bench-store.js). Two locks, because a
// memory store reaching production is silent data loss — no conversations, no requests, no inbox:
// VERCEL is '1' on every deployment, and tools/ is in .vercelignore, so if the env var ever did get
// set out there the require throws MODULE_NOT_FOUND on the first chat instead of quietly forgetting it.
// The return is what makes it stick: the bottom of this file assigns module.exports again.
if (process.env.BENCH_STORE === 'memory' && process.env.VERCEL !== '1') { module.exports = require('../tools/bench-store'); return; }
