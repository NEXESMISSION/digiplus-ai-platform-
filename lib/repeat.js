...
// A short plain answer («Ey, bnina barcha 😊») may be given again word for word: forcing the assistant
// to find other words for it is how it ends up inventing something that isn't Tunisian.
const tooClose = (text, old) => {
  const asks = ASKS.test(String(text).trim());
  const score = sameness(text, old);
  if (score >= 1) return asks || wordsOf(text).length > 5;
  return score >= CLOSE && asks;
};
