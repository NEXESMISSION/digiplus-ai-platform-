...
async function answer(bot, conversation, history) {
  ...
  for (let round = 0; ; round++) {
    ...
    // Before anything goes out: prices must exist, and Derja must be Tunisian. One rewrite if not.
    const issues = guard.problems(bot, { texts: [...texts, ...choices], language, toolResults });
    ...
    if (issues.length) {
      // Still wrong after the rewrite: never send a price that doesn't exist.
