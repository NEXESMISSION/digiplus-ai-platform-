...
async function summary(days = 7) {
  ...
  // One line per ad (?ad=… in the link), so two ads can be compared on real results.
  const ads = new Map();
  for (const session of sessions.values()) {
    ...
    const tag = session.pages.map((p) => p.source).find((s) => s && s.startsWith('ad:'));
    if (!tag) continue;
    const name = tag.slice(3);
    const ad = ads.get(name) || { name, visits: 0, chat: 0, message: 0, request: 0 };
    ad.visits += 1;
    if (opened) ad.chat += 1;
    if (session.events.has('message')) ad.message += 1;
    if (session.events.has('request')) ad.request += 1;
    ads.set(name, ad);
  }
  ...
  return {
    ...
    ads: [...ads.values()].sort((a, b) => b.visits - a.visits),
